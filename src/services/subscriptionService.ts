...
const getOrCreateUserId = async (): Promise<string> => {
  try {
    let userId = await AsyncStorage.getItem('tattoo_app_user_id');
    if (!userId) {
      userId = 'user_' + Math.random().toString(36).substr(2, 9) + Date.now();
      await AsyncStorage.setItem('tattoo_app_user_id', userId);
    }
    return userId;
  } catch (error) {
...
// Initialize user subscription in AsyncStorage
const initializeUserSubscription = async (): Promise<UserSubscription> => {
  ...
  try {
    await AsyncStorage.setItem(SUBSCRIPTION_STORAGE_KEY, JSON.stringify(defaultSubscription));
  } catch (error) {
...
// Get user subscription from AsyncStorage
export const getUserSubscription = async (): Promise<UserSubscription> => {
  try {
    const stored = await AsyncStorage.getItem(SUBSCRIPTION_STORAGE_KEY);
    if (!stored) {
      return await initializeUserSubscription();
    }
...
// Update user subscription in AsyncStorage and sync to Supabase
export const updateUserSubscription = async (updates: Partial<UserSubscription>): Promise<UserSubscription> => {
  ...
  try {
    ...
    await AsyncStorage.setItem(SUBSCRIPTION_STORAGE_KEY, JSON.stringify(updated));
...
// Sync subscription from Supabase to AsyncStorage
export const syncSubscriptionFromSupabase = async (): Promise<UserSubscription> => {
  const localSubscription = await getUserSubscription();
  ...
  try {
    ...
    // If Supabase data is newer, update AsyncStorage
    ...
    if (new Date(supabaseSubscription.lastUpdated) > new Date(localSubscription.lastUpdated)) {
      await AsyncStorage.setItem(SUBSCRIPTION_STORAGE_KEY, JSON.stringify(supabaseSubscription));
...
export const getUserRemainingCredits = async (): Promise<number> => {
  ...
  if (JSON.stringify(subscription) !== JSON.stringify(await getUserSubscription())) {
...
export const canGenerate = async (): Promise<boolean> => {
  const credits = await getUserRemainingCredits();
  return credits > 0;
};
...
export const getCurrentSubscriptionPlan = async (): Promise<SubscriptionPlan | null> => {
  const subscription = await getUserSubscription();
