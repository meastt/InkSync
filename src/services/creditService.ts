...
// Initialize user credits in AsyncStorage
const initializeUserCredits = async (): Promise<UserCredits> => {
  ...
  try {
    await AsyncStorage.setItem(CREDITS_STORAGE_KEY, JSON.stringify(defaultCredits));
  } catch (error) {
...
// Update user credits in AsyncStorage and sync to Supabase
export const updateUserCredits = async (updates: Partial<UserCredits>): Promise<UserCredits> => {
  ...
  try {
    ...
    await AsyncStorage.setItem(CREDITS_STORAGE_KEY, JSON.stringify(updated));
...
// Sync credits from Supabase to AsyncStorage
export const syncCreditsFromSupabase = async (): Promise<UserCredits> => {
  const localCredits = await getUserCredits();
  ...
  try {
    ...
    if (new Date(supabaseCredits.lastUpdated) > new Date(localCredits.lastUpdated)) {
      await AsyncStorage.setItem(CREDITS_STORAGE_KEY, JSON.stringify(supabaseCredits));
...
export const canGenerate = async (): Promise<boolean> => {
  return await subscriptionCanGenerate();
};
...
export const getTotalRemainingCredits = async (): Promise<number> => {
  return await getUserRemainingCredits();
};
...
export const getSubscriptionInfo = async () => {
  ...
  const totalCredits = await getUserRemainingCredits();
